...
FAQ_ITEMS: list[tuple[str, str]] = [
    (
        "Сроки",
        "Сроки зависят от объёма: сценарий и кнопки без внешних сервисов — обычно от нескольких дней; запись, оплаты, CRM, отчёты, ИИ — дольше. Точнее скажу после краткого ТЗ.",
    ),
    (
        "Стоимость",
    ...
    (
        "ИИ в боте",
        "Подсказки и автоответы через OpenAI и аналоги возможны, если есть ключ API и понятные ограничения по темам. Важно продумать лимиты и модерацию.",
    ),
    (
        "Что в ТЗ указать",
